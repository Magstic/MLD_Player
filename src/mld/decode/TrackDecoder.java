...
public final class TrackDecoder {
    public List<DecodedTrack> decodeAll(MldDocument document) throws IOException {
        if (document == null) throw new IllegalArgumentException("MLD document is required.");
        List<DecodedTrack> decodedTracks = new ArrayList<DecodedTrack>();
        for (TrackChunk track : document.tracks) decodedTracks.add(decode(document, track));
    ...
    public DecodedTrack decode(MldDocument file, TrackChunk track) throws IOException {
        List<TrackEvent> events = new ArrayList<TrackEvent>();
        List<String> warnings = new ArrayList<String>();
        byte[] payload = track.copyPayload();
        int offset = 0;
        int rawTick = 0;
